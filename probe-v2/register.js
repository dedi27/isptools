...
const REGISTRATION_INTERVAL = 1 * 60 * 1000; // 30 minutos
...
export async function initializeRegistration() {
    ...
    if (success) {
        ...
        console.log("TESTE");
        console.log(`✗ [${global.sID || process.pid}] Initial registration failed, will retry in ${REGISTRATION_INTERVAL / 1000} seconds`);
    }
    ...
    registrationIntervalId = setInterval(async () => {
        ...
        if (retrySuccess) {
            ...
            console.log(`✗ [${global.sID || process.pid}] Periodic registration failed, will retry in ${REGISTRATION_INTERVAL / 1000} seconds`);
        }
